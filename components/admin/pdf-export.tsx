...
import { AEM_STAMP_BASE64 } from "./stamp";
...
function getElectedSectionTitle(
  ...
  if (normalizedTitle.includes("délégué")) return "Délégués élu(e)s";
  if (normalizedTitle.includes("représentant")) return "Représentants élu(e)s";
  if (normalizedTitle.includes("bureau")) return "Bureau exécutif élu";
...
function addPdfHeader(
  ...
  doc.setFontSize(8);
  doc.text(`Généré le ${generatedAt}`, pageWidth - 22, 24, { align: "right" });

  doc.setFontSize(10);
  doc.text(subtitle, pageWidth - 22, 46, { align: "right" });
}
...
function drawDescriptionBlock(
  ...
  while (remainingLines.length > 0) {
    const availableHeight = pageHeight - bottomMargin - cursorY - paddingY * 2 - labelHeight;
    const maxLinesOnPage = Math.max(1, Math.floor(availableHeight / lineHeight));
    ...
    const blockHeight = linesForThisPage.length * lineHeight + paddingY * 2 + labelHeight;
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(8);
    doc.text("CONTEXTE", margin + paddingX, cursorY + paddingY + 2);
    ...
    linesForThisPage.forEach((line, index) => {
      doc.text(line, margin + paddingX, cursorY + paddingY + labelHeight + index * lineHeight);
    });
...
export function ResultsPdfExport({ results }: { results: ElectionResultSummary[] }) {
  const handleDownload = () => {
    ...
    doc.text(
      `${overallWinner?.isTie ? "Résultat :" : "Gagnant :"} ${overallWinner?.winner ?? "-"}`,
      440,
      90,
    );
    ...
    doc.save(`AEM-MAROC-resultats-${new Date().toISOString().slice(0, 10)}.pdf`);
  };
...
export function SingleResultPdfExport({
  ...
  const handleDownload = () => {
    ...
    const participationRate =
      totalUsers > 0 ? (result.total_votes / totalUsers) * 100 : 0;
    ...
    // Description
    const infoBoxesStartY = drawDescriptionBlock(doc, description, 82, pageWidth);

    // Cases d'information
    let cursorY = drawInfoBoxes(
      doc,
      [
        { label: "Électeurs inscrits", value: String(totalUsers) },
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(12);
    doc.text("Résultats de l'élection", 22, cursorY);
    ...
    let afterResultsY = ((doc as any).lastAutoTable?.finalY ?? cursorY + 10) + 20;
    ...
    if (boardMembers.length > 0) {
      ...
      const electedSectionTitle = getElectedSectionTitle(boardMembers, result.title);

      doc.setTextColor(15, 23, 42);
      doc.setFont("helvetica", "bold");
      doc.setFontSize(12);
      doc.text(electedSectionTitle, 22, afterResultsY);
      ...
      afterResultsY = ((doc as any).lastAutoTable?.finalY ?? afterResultsY + 10) + 12;
    }
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(10);
    doc.text("Commission indépendante des élections", pageWidth / 2, commissionTitleY, {
      align: "center",
    });
    ...
    const commissionMembers = [
      { role: "Président", name: commissionPresident.trim() || "-" },
      { role: "Vice-président", name: commissionVicePresident.trim() || "-" },
      { role: "Rapporteur", name: commissionRapporteur.trim() || "-" },
    ];
    ...
    commissionMembers.forEach((member, index) => {
      ...
      doc.roundedRect(x, cardTopY, columnWidth, cardHeight, 4, 4, "FD");

      doc.setTextColor(15, 23, 42);
      doc.setFont("helvetica", "bold");
      doc.setFontSize(9);
      doc.text(member.role, centerX, cardTopY + 14, { align: "center" });

      const nameLines = doc.splitTextToSize(member.name, columnWidth - 16);
      doc.setTextColor(15, 23, 42);
      doc.setFont("helvetica", "normal");
      doc.setFontSize(8.5);
      ...
      doc.line(x + 12, cardTopY + cardHeight - 18, x + columnWidth - 12, cardTopY + cardHeight - 18);
    });

    /* =====================================================
       CACHET AEM — centré sous la commission
    ===================================================== */

    const stampSize = 80;
    const stampY = cardTopY + cardHeight + 20;
    const stampBottomY = stampY + stampSize;

    let finalStampY = stampY;
    if (stampBottomY > pageHeight - 20) {
      doc.addPage();
      finalStampY = 40;
    }
    ...
    try {
      doc.addImage(
        AEM_STAMP_BASE64,
        "PNG",
        pageWidth / 2 - stampSize / 2,
        finalStampY,
        stampSize,
        stampSize,
      );
    } catch {
      // Si le cachet ne peut pas être chargé, on continue
    }
      ...
      <div className="w-full rounded-xl border border-slate-200 bg-slate-50 p-3 sm:w-72">
        ...
        <div className="space-y-2">
          <input
            ...
            onChange={(event) => setCommissionPresident(event.target.value)}
          ...
          <input
            ...
            onChange={(event) => setCommissionVicePresident(event.target.value)}
          ...
          <input
            ...
            onChange={(event) => setCommissionRapporteur(event.target.value)}
