    ...
    <header className="sticky top-0 z-50 border-b border-slate-200 bg-white/95 backdrop-blur-xl shadow-sm">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 items-center justify-between gap-3 md:justify-center">
          <Link href="/" className="flex min-w-0 items-center gap-3 text-slate-900 md:absolute md:left-4">
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-blue-600 text-white shadow-sm">
              <BarChart3 className="h-5 w-5" />
            </div>
            <span className="truncate text-base font-semibold sm:text-lg">VoteCampus</span>
          </Link>
          ...
          <div className="space-y-2 border-t border-slate-200 bg-white py-3 md:hidden">
              ...
              return (
                <Link
                  ...
                  className={`block rounded-xl px-3 py-2.5 text-sm font-medium transition ${
                    active ? "bg-blue-50 text-blue-600 font-semibold" : "text-gray-600 hover:text-blue-600"
                  }`}
                >
                  {label}
                </Link>
            ...
            {sessionUser && (
              <Link
                ...
                onClick={() => setIsMenuOpen(false)}
                className={`block rounded-xl px-3 py-2.5 text-sm font-medium transition ${
                  isActive("/profile") ? "bg-blue-50 text-blue-600 font-semibold" : "text-gray-600 hover:text-blue-600"
                }`}
              >
                Profil
              </Link>
